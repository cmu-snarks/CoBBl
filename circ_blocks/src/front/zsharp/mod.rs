...
const _NO_OPT: usize = 0;
const OPT_BLOCK_MERGE: usize = 1;
const OPT_SPILLING: usize = 2;
/// The optimization level that enables stack ops
pub const OPT_RO_ARRAYS: usize = 3;

impl FrontEnd for ZSharpFE {
    type Inputs<'ast> = Inputs;
    fn gen(i: Inputs) -> (Computations, usize, usize, Vec<(Vec<usize>, Vec<usize>)>, Vec<usize>, Vec<(usize, usize)>, Vec<bool>) {
        debug!(
        ...
        let (blks, _, io_size, _, live_io_list, num_wit_reads, num_mem_accesses) = 
        ...
        (cs, func_input_width, io_size, live_io_list, num_wit_reads, num_mem_accesses, input_liveness)
    }
}
...
impl ZSharpFE {
    ...
    pub fn interpret(
        ...
        Vec<[Value; 2]>, // Physical memory accesses, sorted by address
        ...
        let init_phy_ops_list = init_phy_ops_list.iter().map(|i|
            [
        ...
        let init_vir_ops_list = init_vir_ops_list.iter().map(|i|
            [
                to_const_value(i.addr_t.clone())
                .unwrap_or_else(|e| panic!("const_entry_fn failed: {}", e)),
                to_const_value(i.data_t.clone())
        ...
        let phy_ops_list = phy_ops_list.iter().map(|i|
            [
                to_const_value(i.addr_t.clone())
                .unwrap_or_else(|e| panic!("const_entry_fn failed: {}", e)),
                to_const_value(i.data_t.clone())
        ...
        let vir_ops_list = vir_ops_list.iter().map(|i|
            [
                to_const_value(i.addr_t.clone())
                .unwrap_or_else(|e| panic!("const_entry_fn failed: {}", e)),
                to_const_value(i.data_t.clone())
        ...
        return (ret, block_id_list, block_outputs_list, block_mems_list, block_io_map_list, init_phy_ops_list, init_vir_ops_list, phy_ops_list, vir_ops_list);
    }

    /// Generate stack and heap transcript for initial memory accesses
    ...
    pub fn generate_input(
        ...
        input_offset: usize,
    ) -> (FxHashMap<usize, Value>, Vec<Value>, Vec<Value>, Vec<[Integer; 2]>, Vec<[Integer; 2]>) {
        // MEMORY ACCESSES
        // Do not reason about input liveness when allocating initial memory
        // Init virtual memory always have ts and ls as 0
        let init_phy_ops_list: Vec<[Integer; 2]> = entry_stacks_concat.iter().enumerate().map(|(i, e)| [Integer::from(i), e.clone()]).collect();
        let init_vir_ops_list: Vec<[Integer; 2]> = entry_arrays_concat.iter().enumerate().map(|(i, e)| [Integer::from(i), e.clone()]).collect();
        let initial_phy_mem = entry_stacks_concat.iter().map(|i| pf_val(i.clone())).collect();
        let initial_vir_mem = entry_arrays_concat.iter().map(|i| pf_val(i.clone())).collect();

        // INPUT MAP
        // Without %BN
        let mut input_indices: Vec<usize> = (0..input_liveness.len() - 2).map(|i| 1 + input_offset + i).collect();
        if input_liveness[1] {
            // %AS is alive, add entry R_AS
            input_indices.insert(0, R_AS);
        }
        if input_liveness[0] {
            // %SP is alive, add entry R_SP
            input_indices.insert(0, R_SP);
        }
        assert_eq!(input_liveness.len(), entry_regs_concat.len());
        let mut live_input = Vec::new();
        for i in 0..input_liveness.len() {
        if input_liveness[i] {
            live_input.push(entry_regs_concat[i].clone());
        }
        }
        input_indices = input_indices[..live_input.len()].to_vec();
        let mut input_map = FxHashMap::with_hasher(Default::default());
        // Add %BN
        input_map.insert(1, pf_val(Integer::from(0)));
        for i in 0..live_input.len() {
            input_map.insert(input_indices[i], pf_val(live_input[i].clone()));
        }
        (input_map, initial_phy_mem, initial_vir_mem, init_phy_ops_list, init_vir_ops_list)
    }
}

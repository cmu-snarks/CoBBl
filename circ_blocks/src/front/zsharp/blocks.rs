...
// These conversions do not need to be bit-checked since they were stored into the memory earlier
fn field_to_ty(f: T, ty: &Ty) -> Result<T, String> {
    match ty {
        Ty::Uint(n) => Ok(T::new(Ty::Uint(*n), term![Op::PfToBvNoCheck(*n); f.term])),
        Ty::Bool => Ok(T::new(Ty::Bool, term![Op::BvBit(0); term![Op::PfToBvNoCheck(1); f.term]])),
        Ty::Field => Ok(f),
        _ => Err(format!("Type not supported: {:?}", ty))
    }
}
...
pub enum BlockContent<'ast> {
    ...
    //      reg    ty   src/dst
    RegRead(usize, Ty, String),
    RegWrite(usize, Ty, Expression<'ast>),
}
...
impl<'ast> ZGen<'ast> {
    ...
    ) -> Result<(Vec<Block<'ast>>, usize, VarScopeInfo), String> {
    ...
    fn bl_gen_function_init_<const IS_MAIN: bool>(
        ...
        f_name: String,
        mut var_scope_info: VarScopeInfo
    ) -> Result<(Vec<Block<'ast>>, usize, Vec<(String, Ty)>, VarScopeInfo), String> {
    ...
    fn bl_gen_function_call_<const IS_MAIN: bool>(
        ...
        func_count: usize,
        mut var_scope_info: VarScopeInfo
    ) -> Result<(Vec<Block<'ast>>, usize, usize, VarScopeInfo, usize), String> {
    ...
    ) -> Result<(Vec<Block<'ast>>, usize), String> {
    ...
    fn bl_gen_expr_<const IS_MAIN: bool>(
        ...
        mut var_scope_info: VarScopeInfo
    ) -> Result<(Vec<Block<'ast>>, usize, VarScopeInfo, Expression<'ast>, usize, usize, usize, usize), String> {
    ...
    fn bl_gen_array_init_<const IS_MAIN: bool>(
        ...
        mut var_scope_info: VarScopeInfo,
    ) -> Result<(Vec<Block<'ast>>, usize, VarScopeInfo, String, usize, usize, usize, usize), String> {
    ...
    fn bl_gen_ref_var(&'ast self, var_name: String) -> T {
        self.identifier_impl_::<false>(&IdentifierExpression {
            value: var_name,
            span: Span::new("", 0, 0).unwrap()
        }).unwrap()
    }
    ...
    fn f_const_t(&'ast self, c: usize) -> T {
        self.literal_(&LiteralExpression::DecimalLiteral(DecimalLiteralExpression {
            ...
            suffix: Some(DecimalSuffix::Field(FieldSuffix { span: Span::new("", 0, 0).unwrap() })),
            span: Span::new("", 0, 0).unwrap()
        })).unwrap()
    }
    ...
    // Returns addr_val if exist
    fn vir_mem_to_circ<const MODE: usize, const ESTIMATE: bool>(
        &'ast self,
        mem_op_count: usize,
        arr: Option<&String>,
        ...
        if !read_only {
            ...
            let ls_val = self.f_const_t(ls);
            self.assign_impl_::<false>(&format!("%tvm{:06}l", mem_op_count), &[][..], ls_val, false).unwrap();
            // TS, increment if STORE
            if MODE == STORE {
                self.stmt_impl_::<false>(&bl_gen_increment_stmt(if ESTIMATE {"%TS"} else {W_TS}, 1, &Ty::Field)).unwrap();
            }
            let ts_val = self.bl_gen_ref_var((if ESTIMATE {"%TS"} else {W_TS}).to_string());
            self.assign_impl_::<false>(&format!("%tvm{:06}t", mem_op_count), &[][..], ts_val, false).unwrap();
        }
        // DATA is updated individually by LOAD or STORE
        // ADDR if exist
        if let Some(index_expr) = id_expr {
            // %ma = index_t + offset_t == rhs_t
            let mut index_t = self.expr_impl_::<false>(&index_expr).unwrap();
            // if addr_t is not a field, cast it to a field
            if index_t.type_() != &Ty::Field {
                index_t = uint_to_field(index_t).unwrap();
            }
            let offset_t = self.bl_gen_ref_var(arr.unwrap().to_string());
            let addr_t = add(index_t, offset_t).unwrap();
            if read_only {
                self.assign_impl_::<false>(&format!("%tpm{:06}a", mem_op_count), &[][..], addr_t.clone(), false).unwrap();
            } else {
                self.assign_impl_::<false>(&format!("%tvm{:06}a", mem_op_count), &[][..], addr_t.clone(), false).unwrap();
            }
            Some(addr_t)
        } else {
            None
        ...
        match i {
            BlockContent::Witness((var, ty, alive)) => {
                ...
                if *alive {
                    ...
                    // Assign VAR to WIT
                    let e = self.bl_gen_ref_var(format!("%wt{:06}", wit_count));
                    ...
                    panic!("Witness statment with variable {} is not alive!", var);
                }
            }
            // Push and pop are not affected by no_ro_accesses
            BlockContent::MemPush((var, _, offset)) => {
                // ADDR
                let addr_val = self.expr_impl_::<false>(&Expression::Binary(BinaryExpression {
                    op: BinaryOperator::Add,
                    left: Box::new(Expression::Identifier(IdentifierExpression {
                        // %SP
                ...
                let addr_var_val = self.circ_declare_input(
                    &f,
                    ...
                    true,
                    &None,
                ).expect("circ_declare addr");
                let addr_eq = eq(addr_val.clone(), addr_var_val).unwrap().term;
                self.circ.borrow_mut().assert(&f, addr_eq);
                // DATA
                // Compute store value, converted to field
                let mut data_val = self.bl_gen_ref_var(var.to_string());
                if data_val.type_() != &Ty::Field {
                    data_val = uint_to_field(data_val).unwrap();
                }
                let prev_val_term = if phy_mem_op_count > num_ro_accesses {
                    self.bl_gen_ref_var(format!("%pm{:06}d", phy_mem_op_count - 1)).term
                } else {
                    self.f_const_t(0).term
                };
                let data_val = T::new(Ty::Field, term![Op::StackPush(default_field(), phy_mem_op_count); self.circ.borrow().condition(), addr_val.term, data_val.clone().term, prev_val_term]);
                let data_var_val = self.circ_declare_input(
                    &f,
                    format!("%pm{:06}d", phy_mem_op_count),
                    &Ty::Field,
                    ZVis::Public,
                    Some(data_val.clone()),
                    true,
                ...
                let data_eq = eq(data_val.clone(), data_var_val).unwrap().term;
                self.circ.borrow_mut().assert(&f, data_eq);
                phy_mem_op_count += 1;
            }
            BlockContent::MemPop((var, ty, offset)) => {
                ...
                let data_val = T::new(Ty::Field, term![Op::StackPop(default_field(), phy_mem_op_count); addr_val.term, prev_val_term]);
                let data_var_val = self.circ_declare_input(
                    &f,
                    format!("%pm{:06}d", phy_mem_op_count),
                    &Ty::Field,
                ...
                let data_val = field_to_ty(data_val, &ty).unwrap();
                self.declare_init_impl_::<false>(
                    var.clone(),
                    ty.clone(),
                    data_val,
            ...
            BlockContent::ArrayInit((arr, _, len_expr, read_only)) => {
                ...
                let mem_pointer = if *read_only { if ESTIMATE {"%SP"} else {W_SP} } else { if ESTIMATE {"%AS"} else {W_AS} };
                // Declare the array as a pointer (field), set to mem_pointer
                let as_t = self.bl_gen_ref_var(mem_pointer.to_string());
                self.declare_init_impl_::<false>(
                    arr.to_string(),
                    Ty::Field,
                    as_t.clone(),
            ...
            BlockContent::Store((val_expr, _, arr, id_expr, init, read_only)) => {
                let read_only = if no_ro_accesses { &false } else { read_only };
                // Compute store value, converted to field
                let mut data_val = self.expr_impl_::<false>(&val_expr).unwrap();
                if data_val.type_() != &Ty::Field {
                    data_val = uint_to_field(data_val).unwrap();
                }
                if *read_only {
                    // ADDR
                    let addr_val = self.vir_mem_to_circ::<STORE, ESTIMATE>(
                        ro_mem_op_count,
                        Some(&arr),
                        Some(&id_expr),
                        *read_only,
                    ).unwrap();
                    // Assign STORE value to data
                    let prev_data_term = if ro_mem_op_count > 0 {
                        self.bl_gen_ref_var(format!("%tpm{:06}d", ro_mem_op_count - 1)).term
                    } else {
                        self.f_const_t(0).term
                    };
                    let store_val = T::new(Ty::Field, term![Op::StackPush(default_field(), ro_mem_op_count); self.circ.borrow().condition(), addr_val.term, data_val.clone().term, prev_data_term]);
                    self.assign_impl_::<false>(&format!("%tpm{:06}d", ro_mem_op_count), &[][..], store_val, false).unwrap();
                    // Update Label
                    ro_mem_op_count += 1;
                } else {
                    // ADDR, LS, & TS
                    let addr_val = if *init {
                        self.vir_mem_to_circ::<INIT_STORE, ESTIMATE>(
                            vir_mem_op_count,
                            Some(&arr),
                            Some(&id_expr),
                            *read_only,
                        ...
                        self.vir_mem_to_circ::<STORE, ESTIMATE>(
                            vir_mem_op_count,
                            Some(&arr),
                            Some(&id_expr),
                            *read_only,
                    ...
                    // Assign STORE value to data
                    let prev_data_term = if vir_mem_op_count > 0 {
                        self.bl_gen_ref_var(format!("%tvm{:06}d", vir_mem_op_count - 1)).term
                    } else {
                        self.f_const_t(0).term
                    };
                    let store_val = T::new(Ty::Field, term![Op::HeapStore(default_field(), vir_mem_op_count); self.circ.borrow().condition(), addr_val.term, data_val.clone().term, prev_data_term]);
                    self.assign_impl_::<false>(&format!("%tvm{:06}d", vir_mem_op_count), &[][..], store_val, false).unwrap();
                    // Update Label
                    vir_mem_op_count += 1;
                };
            }
            BlockContent::Load((val, ty, arr, id_expr, read_only)) => {
                ...
                let mut e = if *read_only {
                    // ADDR
                    let addr_val = self.vir_mem_to_circ::<LOAD, ESTIMATE>(
                        ro_mem_op_count,
                        Some(&arr),
                        Some(&id_expr),
                        *read_only,
                    ...
                    let load_val = T::new(Ty::Field, term![Op::StackPop(default_field(), ro_mem_op_count); addr_val.term, prev_data_term]);
                    self.assign_impl_::<false>(&format!("%tpm{:06}d", ro_mem_op_count), &[][..], load_val, false).unwrap();
                    // Returns %tpmd
                    let e = self.bl_gen_ref_var(format!("%tpm{:06}d", ro_mem_op_count));
                    ro_mem_op_count += 1;
                    e
                } else {
                    // ADDR, LS, & TS
                    ...
                    let load_val = T::new(Ty::Field, term![Op::HeapLoad(default_field(), vir_mem_op_count); addr_val.term, prev_data_term]);
                    self.assign_impl_::<false>(&format!("%tvm{:06}d", vir_mem_op_count), &[][..], load_val, false).unwrap();
                    // Returns %tvmd
                    let e = self.bl_gen_ref_var(format!("%tvm{:06}d", vir_mem_op_count));
                    vir_mem_op_count += 1;
                    e
                };
                // Assign data to var
            ...
            BlockContent::DummyLoad(read_only) => {
                let read_only = if no_ro_accesses { &false } else { read_only };
                // ADDR, LS, & TS
                self.vir_mem_to_circ::<DUMMY_LOAD, ESTIMATE>(
                    if *read_only { ro_mem_op_count } else { vir_mem_op_count },
                    None,
                    None,
                    *read_only,
                );
                let addr_val = self.f_const_t(0);
                if *read_only {
                    // Assign LOAD value to data
                    let prev_data_term = if ro_mem_op_count > 0 {
                        self.bl_gen_ref_var(format!("%tpm{:06}d", ro_mem_op_count - 1)).term
            ...
            BlockContent::Branch((cond, if_insts, else_insts)) => {
                ...
                for i in if_insts {
                    (if_wit_count, if_phy_mem_op_count, if_ro_mem_op_count, if_vir_mem_op_count) = self.inst_to_circ::<ESTIMATE>(i, f, wit_count, if_phy_mem_op_count, if_ro_mem_op_count, if_vir_mem_op_count, num_ro_accesses, no_ro_accesses);
                }
                self.circ_exit_condition();
                self.circ_enter_condition(term![NOT; cbool]);
                for i in else_insts {
                    (else_wit_count, else_phy_mem_op_count, else_ro_mem_op_count, else_vir_mem_op_count) = self.inst_to_circ::<ESTIMATE>(i, f, wit_count, else_phy_mem_op_count, else_ro_mem_op_count, else_vir_mem_op_count, num_ro_accesses, no_ro_accesses);
                }
                self.circ_exit_condition();
            ...
            BlockContent::RegRead(r, ty, src) => {
                if ESTIMATE {
                    unreachable!()
                } else {
                    // ty src = %i{r}
                    // Assign VAR to WIT
                    let e = self.bl_gen_ref_var(format!("%i{:06}", r));
                    self.declare_init_impl_::<false>(
                        src.clone(),
                        ty.clone(),
                        e,
            ...
            BlockContent::RegWrite(r, ty, dst) => {
                if ESTIMATE {
                    unreachable!()
                } else {
                    let output_ty = if let Ty::Array(..) = ty { &Ty::Field } else { ty };
                    let output_name = format!("%o{:06}", r);
                    let output_val = self.expr_impl_::<false>(dst).unwrap();
                    let output_var_val = self
                        .circ_declare_input(&f, output_name, output_ty, ZVis::Public, Some(output_val.clone()), true, &None)
                        .expect("circ_declare return");
                    let ret_eq = eq(output_val, output_var_val).unwrap().term;
                    self.circ.borrow_mut().assert(&f, ret_eq);
                }
            }
        }
    ...
    pub fn bl_to_circ<const ESTIMATE: bool>(&self, b: &Block, f: &str, no_ro_accesses: bool) {
        ...
        // Assign a variable to all stack and heap accesses to account for different branches
        let mem_access_default = self.f_const_t(0);
        for i in 0..num_ro_ops {
            // Declare the next temporary ADDR and DATA
            self.declare_init_impl_::<false>(format!("%tpm{:06}a", i), Ty::Field, mem_access_default.clone()).unwrap();
            self.declare_init_impl_::<false>(format!("%tpm{:06}d", i), Ty::Field, mem_access_default.clone()).unwrap();
        }
        // Declare all virtual memory accesses to avoid dealing with branching
        for i in 0..num_vm_ops {
            // Declare the next temporary ADDR, DATA, L/S, and TS
            self.declare_init_impl_::<false>(format!("%tvm{:06}a", i), Ty::Field, mem_access_default.clone()).unwrap();
            self.declare_init_impl_::<false>(format!("%tvm{:06}d", i), Ty::Field, mem_access_default.clone()).unwrap();
            self.declare_init_impl_::<false>(format!("%tvm{:06}l", i), Ty::Field, mem_access_default.clone()).unwrap();
            self.declare_init_impl_::<false>(format!("%tvm{:06}t", i), Ty::Field, mem_access_default.clone()).unwrap();
        }

        // Iterate over instructions, convert memory accesses into statements and then IR
        for i in &b.instructions {
            (wit_count, phy_mem_op_count, ro_mem_op_count, vir_mem_op_count) = 
                self.inst_to_circ::<ESTIMATE>(i, f, wit_count, phy_mem_op_count, ro_mem_op_count, vir_mem_op_count, b.num_ro_ops, no_ro_accesses);
        }
        
        // Declare all read-only memory accesses to avoid dealing with branching
        let mem_decl_func = |i: usize, prefix: &str, suffix: &str| {
            let mem_val = self.bl_gen_ref_var(format!("%t{}m{:06}{}", prefix, i, suffix));
            let mem_var_val = self.circ_declare_input(
                f,
                format!("%t{}m{:06}{}", prefix, i, suffix),
                &Ty::Field,
                ZVis::Public,
                Some(mem_val.clone()),
                true,
                &None,
            ).expect("circ_declare mem");
            let mem_eq = eq(mem_val, mem_var_val).unwrap().term;
            self.circ.borrow_mut().assert(&f, mem_eq);
        };
        for i in 0..num_ro_ops {
        ...
        if let Some(_) = self.circ_exit_fn() {
            match self.mode {
                Mode::Proof => {
                    let assertions = std::mem::take(&mut *self.assertions.borrow_mut());
                    let to_assert = term(AND, assertions);

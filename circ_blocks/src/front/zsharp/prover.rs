...
impl<'ast> ZGen<'ast> {
    ...
    pub fn bl_eval_entry_fn<const VERBOSE: bool>(
        ...
        let mut init_phy_ops_list: Vec<MemOp> = Vec::new();
        let mut init_vir_ops_list: Vec<MemOp> = Vec::new();
                ...
                Ty::Array(read_only, _, entry_ty) => {
                    ...
                    if *read_only {
                        ...
                        for entry in entry_stack {
                            ...
                            init_phy_ops_list.push(MemOp::new_phy(addr, addr_t, data_t));
                            stack_addr_count += 1;
                        ...
                        for entry in entry_array {
                            ...
                            init_vir_ops_list.push(MemOp::new_vir(addr, addr_t, data_t, ls_t, ts, ts_t));
                            mem_addr_count += 1;
                            next_ty_index = (next_ty_index + 1) % entry_ty.len();
                        }
                    }
        ...
        let (phy_ops_list, vir_ops_list) = sort_by_mem(&init_phy_ops_list, &init_vir_ops_list, &bl_exec_state);
        Ok((ret?, bl_exec_count, prog_reg_in, bl_exec_state, init_phy_ops_list, init_vir_ops_list, phy_ops_list, vir_ops_list))
    }
    ...
    fn bl_eval_inst_impl_(
        ...
        for s in inst {
            ...
            match s {
                ...
                BlockContent::RegRead(r, _, dst) => {
                    let reg_t = self.bl_eval_identifier_impl_(io_regs, wit_regs, &format!("%i{:06}", r))?;
                    self.bl_eval_assign_impl_(io_regs, wit_regs, dst, reg_t)?;
                }
                BlockContent::RegWrite(r, _, src) => {
                    let dst_t = self.bl_eval_expr_impl_(io_regs, wit_regs, src)?;
                    let reg = &format!("%o{:06}", r); 
                    self.bl_eval_assign_impl_(io_regs, wit_regs, reg, dst_t)?;
                }
            }
        };
        Ok(witness_count)
...
pub fn sort_by_mem(
    init_phy_ops_list: &Vec<MemOp>,
    init_vir_ops_list: &Vec<MemOp>,
    bl_exec_state: &Vec<ExecState>,
) -> (Vec<MemOp>, Vec<MemOp>) {
    let mut sorted_phy_mem_op_list = init_phy_ops_list.clone();
    let mut sorted_vir_mem_op_list = init_vir_ops_list.clone();

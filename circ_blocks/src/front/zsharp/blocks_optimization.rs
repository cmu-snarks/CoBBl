...
fn la_inst<'ast, const ELIM: bool>(
    ...
    for i in inst.iter().rev() {
        match i {
            ...
            BlockContent::RegRead(..) | BlockContent::RegWrite(..) => { panic!("Blocks pending optimization should not contain Reg instructions.") }
        }
    }
...
fn tta_inst<'ast, const IN_BRANCH: bool>(
    ...
    for i in inst {
        match i {
            ...
            BlockContent::RegRead(_, _, dst) => {
                gen_set.insert(dst.clone());
                new_instr.push(i.clone());
            }
            BlockContent::RegWrite(r, _, _) => {
                let var = format!("%o{:06}", r);
                gen_set.insert(var);
                new_instr.push(i.clone());
            }
...
fn bmc_inst<'ast>(
    ...
    for i in inst {
        match i {
            ...
            */
            BlockContent::Branch((_, if_inst, else_inst)) => {
                let (if_wit_reads_count, if_phy_mem_accesses_count, if_vir_mem_accesses_count) = bmc_inst(&if_inst, no_ro_accesses);
                let (else_wit_reads_count, else_phy_mem_accesses_count, else_vir_mem_accesses_count) = bmc_inst(&else_inst, no_ro_accesses);
                // Wit reads of both branches should always be the same
                assert_eq!(if_wit_reads_count, else_wit_reads_count);
                // Through dummy loads, ro and mem ops of both branches should be the same
                ...
                wit_reads_count += if_wit_reads_count;
                phy_mem_accesses_count += if_phy_mem_accesses_count;
                vir_mem_accesses_count += if_vir_mem_accesses_count;
            }
            BlockContent::Stmt(_) => {}
            BlockContent::RegRead(..) | BlockContent::RegWrite(..) => {}
        }
    }
    (wit_reads_count, phy_mem_accesses_count, vir_mem_accesses_count)
}
...
impl<'ast> ZGen<'ast> {
    ...
    pub fn process_block<const MODE: usize>(
        ...
        verbose: bool,
        // inputs: Vec<(String, Ty)>,
    ) -> (Vec<Block<'ast>>, usize, usize, usize, Vec<(Vec<usize>, Vec<usize>)>, Vec<usize>, Vec<(usize, usize)>) {
        println!("\n\n--\nPost-Processing:");
        ...
        */
        ...
        // VtR
        let (bls, transition_map_list, io_size, witness_map, witness_size, live_io) = self.var_to_reg(bls, &predecessor, &successor, entry_bl);
        if verbose {
            println!("\n\n--\nVar -> Reg:");
            println!("Var -> IO map:");
        ...
        let (num_wit_reads, num_mem_accesses) = self.get_blocks_memory_info(&bls, opt_level < OPT_RO_ARRAYS);

        print_bls(&bls, &entry_bl);
        (bls, entry_bl, io_size, witness_size, live_io, num_wit_reads, num_mem_accesses)
    }
    ...
    fn var_to_reg(
        ...
        for i in 0..bls.len() {
            ...
            for (name, ty) in &bls[i].inputs {
                ...
                new_instr.push(BlockContent::RegRead(live_input_label, ty.clone().unwrap(), new_witness_name));
            }
            ...
            for (name, ty) in &bls[i].outputs {
                ...
                let new_witness_name: String;
                (new_witness_name, witness_map, _) = var_name_to_reg_id_expr::<0>(name.to_string(), witness_map);
                let new_witness_expr = Expression::Identifier(IdentifierExpression {
                    value: new_witness_name,
                    span: Span::new("", 0, 0).unwrap()
                });
                new_instr.push(BlockContent::RegWrite(live_output_label, ty.clone().unwrap(), new_witness_expr));
            }
            ...
            new_instr.push(BlockContent::RegWrite(R_BN, Ty::Field, new_expr));
    ...
    // Returns:
    // 0. # of witness reads per block
    // 1. # of (physical (scoping) memory, virtual memory accesses) accesses per block
    ...
    fn get_blocks_memory_info(
        ...
        // Number of witness reads per block
        let mut num_wit_reads = Vec::new();
        // Number of memory accesses per block
        let mut num_mem_accesses = Vec::new();
        for b in bls {
            let (wit_reads_count, phy_mem_accesses_count, vir_mem_accesses_count) = bmc_inst(&b.instructions, no_ro_accesses);
            num_wit_reads.push(wit_reads_count);
            num_mem_accesses.push((phy_mem_accesses_count, vir_mem_accesses_count));
        }
        (num_wit_reads, num_mem_accesses)
    }

        ...
        if let Sort::BitVector(n) = check(&bv) {
            if !self.cache.contains_key(&bv) {
                match &bv.op() {
                    ...
                    Op::PfToBvNoCheck(nbits) => {
                        let lc = self.get_pf(&bv.cs()[0]).clone();
                        self.set_bv_uint(bv.clone(), lc, *nbits);
                    }
                    Op::BoolToBv => {
    ...
    fn embed_pf(&mut self, c: Term) -> &TermLc {
        ...
        if !self.cache.contains_key(&c) {
            ...
            let lc = match &c.op() {
                ...
                Op::StackPush(_, _) => self.fresh_wit("push_data", self.get_pf(&c.cs()[2]).0.clone()),
                Op::StackPop(_, _) => self.fresh_wit("pop_data", c.clone()),
                Op::HeapStore(_, _) => self.fresh_wit("store_data", self.get_pf(&c.cs()[2]).0.clone()),
                Op::HeapLoad(_, _) => self.fresh_wit("load_data", c.clone()),

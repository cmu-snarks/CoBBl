...
fn check_dependencies(t: &Term) -> Vec<Term> {
    match &t.op() {
        ...
        Op::StackPush(..) => vec![t.cs()[3].clone()],
        Op::StackPop(..) => vec![t.cs()[1].clone()],
        Op::HeapStore(..) => vec![t.cs()[3].clone()],
        Op::HeapLoad(..) => vec![t.cs()[1].clone()],
    }
}
...
fn check_raw_step(t: &Term, tys: &TypeTable) -> Result<Sort, TypeErrorReason> {
    ...
    match &t.op() {
        ...
        Op::PfToBvNoCheck(a) => Ok(Sort::BitVector(*a)),
        ...
        Op::StackPush(m, _) => Ok(Sort::Field(m.clone())),
        Op::StackPop(m, _) => Ok(Sort::Field(m.clone())),
        Op::HeapStore(m, _) => Ok(Sort::Field(m.clone())),
        Op::HeapLoad(m, _) => Ok(Sort::Field(m.clone())),
...
pub fn rec_check_raw_helper(oper: &Op, a: &[&Sort]) -> Result<Sort, TypeErrorReason> {
    match (oper, a) {
        ...
        (Op::PfToBvNoCheck(a), &[Sort::Field(_)]) => Ok(Sort::BitVector(*a)),

...
/// An operator
pub enum Op {
    /// read from a transition register, cast to type Sort
    Reg(usize, Sort),
    /// read from a witness transcript, cast to type Sort
    Witness(usize, Sort),
    /// a variable
    Var(String, Sort),
    /// a constant
    ...
    PfToBv(usize),
    /// translate a prime-field element into a certain-width bit-vector, without checking for bits
    PfToBvNoCheck(usize),
    ...
    /// Stack Operations, on a write-once array
    /// Uses last_val to ensure that array accesses are performed in sequence
    /// Binary operator with arguments (cond, addr, val, last_val), sets %PHY[addr] = val, outputs val
    StackPush(FieldT, usize),
    /// Unary operator with arguments (addr, last_val), outputs %PHY[addr]
    StackPop(FieldT, usize),
    /// Heap Operations
    /// Binary operator with arguments (cond, addr, val, last_val), sets %VIR[addr] = val, outputs val
    HeapStore(FieldT, usize),
    /// Unary operator with arguments (addr, last_val), outputs %VIR[addr]
    HeapLoad(FieldT, usize),
}
...
impl Term {
    ...
    pub fn is_mem(&self) -> bool {
        matches!(&self.op(), Op::StackPush(..) | Op::StackPop(..) | Op::HeapStore(..) | Op::HeapLoad(..))
    }
...
/// Helper function for eval function in block format. Handles a single op
#[allow(clippy::uninlined_format_args)]
pub fn eval_op_block(
    op: &Op, 
    args: &[&Value], 
    entry_witnesses: &Vec<Integer>,
    witness_offset: usize,
    trans_regs: &Vec<Value>,
    phy_mem: &Vec<Value>,
    phy_mem_patches: &HashMap<usize, Value>,
    vir_mem: &Vec<Value>,
    vir_mem_patches: &HashMap<usize, Value>,
) -> Value {
    let var = match op {
        Op::Reg(i, s) => {
            let val = &trans_regs[*i];
            if let Sort::Field(_) = val.sort() {} else { panic!("Non-field transition registers not supported: {:?}!", val.sort()) }
            match s {
                Sort::Field(_) | Sort::Array(..) => val.clone(),
                Sort::BitVector(width) => {eval_op(&Op::PfToBv(*width), &[val], &Default::default())}
                Sort::Bool => {eval_op(&Op::PfToBoolTrusted, &[val], &Default::default())}
                _ => panic!("Unsupported register type!")
            }
        }
        Op::Var(_, _) => { panic!("Vars not supported") }
        Op::Eq => Value::Bool(args[0] == args[1]),
        Op::Not => Value::Bool(!args[0].as_bool()),
        Op::Implies => Value::Bool(!args[0].as_bool() || args[1].as_bool()),
        Op::BoolNaryOp(BoolNaryOp::Or) => Value::Bool(args.iter().any(|a| a.as_bool())),
        ...
        Op::PfToBv(w) | Op::PfToBvNoCheck(w) => Value::BitVector({
            let i = args[0].as_pf().i();
            if let FieldToBv::Panic = cfg().ir.field_to_bv {
                assert!(
                    (i.significant_bits() as usize) <= *w,
        ...
        Op::PfChallenge(name, field) => Value::Field(pf_challenge(name, field)),
        Op::Witness(i, sort) => {
            let wit_int = entry_witnesses[witness_offset + *i].clone();
            let wit_val = match sort {
                Sort::Bool => Value::Bool(wit_int == 1),
                Sort::Field(_) | Sort::Array(..) => Value::Field(cfg().field().new_v(wit_int)),
                Sort::BitVector(width) => Value::BitVector(BitVector::new(wit_int, *width)),
                _ => panic!("Witness type {:?} not supported!", sort)
            };
            wit_val
        }
        Op::PfFitsInBits(n_bits) => {
        ...
        // tuple
        Op::Tuple | Op::Field(_) | Op::Update(_) => { panic!("Tuples not supported") }
        // array
        Op::Store | Op::CStore | Op::Fill(..) | Op::Select | Op::Map(_) | Op::Rot(_) | Op::PfToBoolTrusted | Op::ExtOp(_) => { panic!("Array ops not supported") }
        Op::Array(key, value) => Value::Array(Array::from_vec(
            key.clone(),
            value.clone(),
            args.iter().cloned().cloned().collect(),
        ...
        Op::StackPop(fty, _) => {
            let stack_index = args[0].as_usize().unwrap();
            // First check if stack_index has been patched
            let default_val = Value::Field(fty.default_value());
            phy_mem_patches.get(&stack_index).unwrap_or(
                if stack_index >= phy_mem.len() {
                    &default_val // If index is out of bound, assume the access is a dummy
                } else {
                    &phy_mem[stack_index]
                }
            ).clone()
        ...
        Op::HeapLoad(fty, _) => {
            let heap_index = args[0].as_usize().unwrap();
            // First check if heap_index has been patched
            let default_val = Value::Field(fty.default_value());
            vir_mem_patches.get(&heap_index).unwrap_or_else(||
                if heap_index >= vir_mem.len() { 
                    &default_val // If index is out of bound, assume the access is a dummy
                } else {
                    &vir_mem[heap_index] 
                }

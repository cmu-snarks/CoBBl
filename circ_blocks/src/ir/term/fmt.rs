...
impl DisplayIr for Op {
    fn ir_fmt(&self, f: &mut IrFormatter) -> FmtResult {
        match self {
            ...
            Op::PfToBvNoCheck(a) => write!(f, "(pf2bvnc {a})"),
            Op::Implies => write!(f, "=>"),
            ...
            Op::Witness(i, _) => write!(f, "(witness {i})"),
            ...
            Op::StackPush(_, count) => write!(f, "(push {})", count),
            Op::StackPop(_, count) => write!(f, "(pop {})", count),
            Op::HeapStore(_, count) => write!(f, "(store {})", count),
            Op::HeapLoad(_, count) => write!(f, "(load {})", count),

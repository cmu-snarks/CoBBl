...
impl EqPolynomial {
  ...
  pub fn compute_factored_lens(ell: usize) -> (usize, usize) {
    if ell < (2 * W3_WIDTH).log_2() {
      (ell / 2, ell - ell / 2)
    } else {
      // Minimum right_num_vars used by shift proof
      let right_num_vars = max((2 * W3_WIDTH).log_2(), ell - ell / 2);
...
impl PolyEvalProof {
  ...
  pub fn verify_plain_batched_polys(
    proof_list: &Vec<PolyEvalProof>,
    gens: &PolyCommitmentGens,
    transcript: &mut Transcript,
    r_list: Vec<&Vec<Scalar>>,  // point at which the polynomial is evaluated
  ...
  // Like prove_batched_polys, but r is divided into rq ++ ry
  // Each polynomial is supplemented with num_proofs and num_inputs
  pub fn prove_batched_polys_disjoint_rounds(
  ...
  pub fn verify_plain_batched_polys_disjoint_rounds(
    proof_list: &Vec<PolyEvalProof>,
    num_proofs_list: &Vec<usize>,
    num_inputs_list: &Vec<usize>,
    gens: &PolyCommitmentGens,
  ...
  pub fn prove_uni_batched_polys(
    poly_list: &Vec<&DensePolynomial>,
    r: &Scalar,                    // point at which the polynomial is evaluated
  ...
  pub fn verify_uni_batched_polys(
    &self,
    gens: &PolyCommitmentGens,
    transcript: &mut Transcript,
    r: &Scalar,              // point at which the polynomial is evaluated
  ...
  // Prove that a list of polynomials A are all of form [A0, A1, A1, A2, A2, A3, ... A{N-1}]
  // Where each A0, A1, ... is of length W is fixed and N can be dynamic
  // Achieved through two vector products given a random input R = (r, r^2, ..., r^W)
  // - E0 = A * [0, R, 0, R^W, 0, R^2W, ...]
  // - E1 = A * [R, 0, R^W, 0, R^2W, 0, ...]
  // For hyrax, this is equivalent to setting
  // - W = [1, R^K, R^2K, R^3K, ...], where K = sqrt(W * N / 2)
  // - R0 = [0, R, 0, R^W, 0, R^2W, ...]
  // - R1 = [R, 0, R^W, 0, R^2W, 0, ...]
  // Returns opening proof, and (Zr0, Zr1)
  pub fn prove_row_shift_batched(
    poly_list: &Vec<&DensePolynomial>,
    W: usize,
    N_list: Vec<usize>,
    gens: &PolyCommitmentGens,
    transcript: &mut Transcript,
    random_tape: &mut RandomTape,
  ) -> (PolyEvalProof, Vec<Vec<CompressedGroup>>, (Vec<CompressedGroup>, Vec<CompressedGroup>)) {
    transcript.append_protocol_name(PolyEvalProof::protocol_name());

    for i in 0..poly_list.len() {
      assert_eq!(poly_list[i].len(), (2 * W * N_list[i]).next_power_of_two());
    }

    let max_num_vars = poly_list.iter().fold(0, |m, p| if p.get_num_vars() > m { p.get_num_vars() } else { m });
    let zero = Scalar::zero();

    // W differs depending on size of the polynomial, but R always stay the same
    let (_, right_num_vars) = EqPolynomial::compute_factored_lens(max_num_vars);
    let R_size = right_num_vars.pow2();
    assert_eq!(R_size % (2 * W), 0); // R_size should be divisible by 2 * W

    // compute R0 = <0, ..., 0, r, r^2, ..., r^W, 0, ..., 0, r^{W+1}, ..., r^{2W}, 0, ...>
    // and R1 = <r, r^2, ..., r^W, 0, ..., 0, r^{W+1}, ..., r^{2W}, 0, ...>
    let r = transcript.challenge_scalar(b"challenge_r");
    let (R0, R1) = {
      let mut r_base = r;
      let mut R0 = Vec::new();
      let mut R1 = Vec::new();
      for _ in 0..R_size / (2 * W) {
        let mut next_R = Vec::new(); // r^{i * W + 1}, ..., r^{(i + 1) * W}
        for _ in 0..W {
          next_R.push(r_base);
          r_base *= r;
        }
        R0.extend(vec![zero; W]);
        R0.extend(&next_R);
        R1.extend(&next_R);
        R1.extend(vec![zero; W]);
      }
      (R0, R1)
    };
    let mut L_map: HashMap<usize, Vec<Scalar>> = HashMap::new();

    // Compute the header of each poly
    let comm_header: Vec<Vec<CompressedRistretto>> = poly_list.iter().map(|poly| 
      (0..W).map(|i| poly[i].commit(&zero, &gens.gens.gens_1).compress()).collect()
    ).collect();
    for p in &comm_header {
      for hc in p {
        hc.append_to_transcript(b"comm_header", transcript);
      }
    }

    // Compute and commit Zr0, Zr1 of each poly
    let mut C_Zr0_list: Vec<CompressedGroup> = Vec::new();
    let mut C_Zr1_list: Vec<CompressedGroup> = Vec::new();
    // compute the vector underneath L*Z
    // compute vector-matrix product between L and Z viewed as a matrix
    let c_base = transcript.challenge_scalar(b"challenge_c");
    let mut c = Scalar::one();
    ...
    for i in 0..poly_list.len() {
      ...
      let L = if let Some(L) = L_map.get(&num_vars) { L } else {
        ...
        let L_size = left_num_vars.pow2();
        let R_size = right_num_vars.pow2();
        let r_base = (0..R_size / 2).fold(Scalar::one(), |p, _| p * r); // Half of the R_size is 0
        // L is 1, r^k, r^2k, ...
        let mut l_base = Scalar::one();
        let mut L = Vec::new();
        for _ in 0..L_size {
      ...
      let LZ = poly.bound(&L);
      let Zr0: Scalar = zip(&LZ, &R0).map(|(a, b)| a * b).sum();
      let Zr1: Scalar = zip(&LZ, &R1).map(|(a, b)| a * b).sum();
      LZ_comb = (0..R_size).map(|i| LZ_comb[i] + if i < LZ.len() { c * LZ[i] } else { zero }).collect();
      Zr_comb += c * Zr0 + c * c_base * Zr1;
      c *= c_base * c_base;
      C_Zr0_list.push(Zr0.commit(&zero, &gens.gens.gens_1).compress());
      C_Zr1_list.push(Zr1.commit(&zero, &gens.gens.gens_1).compress());

      // Prove shift
      let header: Scalar = (0..W).map(|i| poly[i] * R1[i]).sum();
      let r_factor = (0..W).fold(Scalar::one(), |p, _| p * r);
      assert_eq!(Zr0 * r_factor + header, Zr1);
    }

    // Prove opening
    let R_comb: Vec<Scalar> = zip(R0, R1).map(|(r0, r1)| r0 + c_base * r1).collect();
    let (proof, _C_LR, _C_Zr_prime) = DotProductProofLog::prove(
      &gens.gens,
      transcript,
      random_tape,
    ...
    (PolyEvalProof { proof }, comm_header, (C_Zr0_list, C_Zr1_list))
  }

  pub fn verify_row_shift_batched(
    &self,
    comm_list: &Vec<&PolyCommitment>,
    W: usize,
    N_list: Vec<usize>,
    comm_header: &Vec<Vec<CompressedGroup>>,
    evals: (&Vec<CompressedGroup>, &Vec<CompressedGroup>),
    gens: &PolyCommitmentGens,
    transcript: &mut Transcript,
  ) -> Result<(), ProofVerifyError> {
    transcript.append_protocol_name(PolyEvalProof::protocol_name());

    let poly_size: Vec<usize> = N_list.iter().map(|N| (2 * W * N).next_power_of_two()).collect();
    let max_poly_size = poly_size.iter().max().unwrap();
    let max_num_vars = max_poly_size.log_2();
    let zero = Scalar::zero();
    // compute L and R
    let (_, right_num_vars) = EqPolynomial::compute_factored_lens(max_num_vars);
    let R_size = right_num_vars.pow2();
    ...
    for i in 0..comm_list.len() {
      ...
      let C_LZ = GroupElement::vartime_multiscalar_mul(L, C_decompressed);
      C_LZ_comb += c * C_LZ;
      C_Zr_comb += c * C_Zr0[i].decompress().unwrap() + c * c_base * C_Zr1[i].decompress().unwrap();
      c *= c_base * c_base;

      // Verify shift
      let header: RistrettoPoint = (0..W).map(|j| comm_header[i][j].decompress().unwrap() * R1[j]).sum();
      assert_eq!(C_Zr0[i].decompress().unwrap() * r_factor + header, C_Zr1[i].decompress().unwrap());
    }
    ...
    self
      .proof
      .verify(R_size, &gens.gens, transcript, &R_comb, &C_LZ_comb.compress(), &C_Zr_comb.compress())
  }
}

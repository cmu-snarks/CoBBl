...
impl SumcheckInstanceProof {
  ...
  pub fn prove_cubic_batched<F>(
    ...
    comb_func: F,
    transcript: &mut Transcript,
  ) -> (Self, Vec<Scalar>, (Vec<Scalar>, Vec<Scalar>, Scalar), (Vec<Scalar>, Vec<Scalar>, Vec<Scalar>))
  where
    F: Fn(&Scalar, &Scalar, &Scalar) -> Scalar,
  {
  ...
  pub fn prove_cubic_disjoint_rounds<F>(
    ...
    comb_func: F,
    transcript: &mut Transcript,
  ) -> (Self, Vec<Scalar>, Vec<Scalar>, Vec<Vec<Scalar>>)
  where
    F: Fn(&Scalar, &Scalar, &Scalar) -> Scalar,
  {
    ...
    let mut r: Vec<Scalar> = Vec::new();
    let mut polys: Vec<CompressedUniPoly> = Vec::new();

    ...
    ProofTranscript: Transcript,
{
    ...
    fn verify(
        ...
        let mut preprocess_il_stm_bytes = Vec::new();
        preprocessing.instruction_lookups.subtable_to_memory_indices.serialize_compressed(&mut preprocess_il_stm_bytes).unwrap();
        let mut preprocess_il_itm_bytes = Vec::new();
        preprocessing.instruction_lookups.instruction_to_memory_indices.serialize_compressed(&mut preprocess_il_itm_bytes).unwrap();
        let mut preprocess_il_mts_bytes = Vec::new();
        preprocessing.instruction_lookups.memory_to_subtable_index.serialize_compressed(&mut preprocess_il_mts_bytes).unwrap();
        let mut preprocess_il_mtd_bytes = Vec::new();
        preprocessing.instruction_lookups.memory_to_dimension_index.serialize_compressed(&mut preprocess_il_mtd_bytes).unwrap();
        let preprocess_il_len = preprocess_il_stm_bytes.len() + preprocess_il_itm_bytes.len() + preprocess_il_mts_bytes.len() + preprocess_il_mtd_bytes.len();
        let mut preprocess_b_bytes = Vec::new();
        preprocessing.bytecode.v_init_final.serialize_compressed(&mut preprocess_b_bytes).unwrap();
        let mut preprocess_rwm_bytes = Vec::new();
        preprocessing.read_write_memory.program_io.serialize_compressed(&mut preprocess_rwm_bytes).unwrap();
        // println!("{} + {} + {}", preprocess_il_len, preprocess_b_bytes.len(), preprocess_rwm_bytes.len());
        let preprocess_size = preprocess_il_len + preprocess_b_bytes.len() + preprocess_rwm_bytes.len();
        
        let mut proof_bytes = Vec::new();
        proof.serialize_compressed(&mut proof_bytes).unwrap();
        let proof_size = proof_bytes.len();
        let mut commit_bytes = Vec::new();
        commitments.serialize_compressed(&mut commit_bytes).unwrap();
        let commit_size = commit_bytes.len();
        println!("{}", preprocess_size + proof_size + commit_size);

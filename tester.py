...
def execute_cobbl_no_opt(b_name, f_name, perc):
    b_name += "_cobbl"
    print("COBBL - OPT_0")
    os.system(f"echo 'COBBL_OPT_0 {perc}' >> {f_name}")
    os.system(f"timeout {TIMEOUT} bash -c \"cd circ_blocks && target/release/examples/zxc --opt_level=0 benchmarks/{b_name} | \
            sed -n 's/Compiler time: //p' \
                >> ../{f_name}\"")
    os.system(f"timeout {TIMEOUT} bash -c \"cd spartan_parallel && RUST_BACKTRACE=1 target/release/examples/interface benchmarks/{b_name} | \
                sed -n -e 's/Preprocess time: //p' \
                ...
                >> ../{f_name}\"")
    print("COBBL - OPT_1")
    os.system(f"echo 'COBBL_OPT_1 {perc}' >> {f_name}")
    os.system(f"timeout {TIMEOUT} bash -c \"cd circ_blocks && target/release/examples/zxc --opt_level=1 benchmarks/{b_name} | \
            sed -n 's/Compiler time: //p' \
                >> ../{f_name}\"")
    os.system(f"timeout {TIMEOUT} bash -c \"cd spartan_parallel && RUST_BACKTRACE=1 target/release/examples/interface benchmarks/{b_name} | \
                sed -n -e 's/Preprocess time: //p' \
                ...
                >> ../{f_name}\"")
    print("COBBL - OPT_2")
    os.system(f"echo 'COBBL_OPT_2 {perc}' >> {f_name}")
    os.system(f"timeout {TIMEOUT} bash -c \"cd circ_blocks && target/release/examples/zxc --opt_level=2 benchmarks/{b_name} | \
            sed -n 's/Compiler time: //p' \
                >> ../{f_name}\"")
    os.system(f"timeout {TIMEOUT} bash -c \"cd spartan_parallel && RUST_BACKTRACE=1 target/release/examples/interface benchmarks/{b_name} | \
                sed -n -e 's/Preprocess time: //p' \

...
use libspartan::{Assignment, InputsAssignment, Instance, SNARKGens, VarsAssignment, SNARK, ComputationCommitment};
...
pub fn prove<P: AsRef<Path>>(
...
) -> io::Result<(SNARKGens, Instance, SNARK, ComputationCommitment)> {
    let prover_data = read_prover_data::<_>(p_path)?;

    println!("Converting R1CS to Spartan");
    let (inst, wit, inps, num_cons, num_vars, num_inputs, num_nz_entries) =
    ...
    let pf = SNARK::prove(&inst, &comm, &decomm, wit, &inps, &gens, &mut prover_transcript);

    Ok((gens, inst, pf, comm))
}
...
pub fn r1cs_to_spartan(
    ...
    let mut num_nz_entries_a = 0; // # of non-zero entries for A, B, C
    let mut num_nz_entries_b = 0;
    let mut num_nz_entries_c = 0;
    ...
    println!("num_nz_entries_a: {}", num_nz_entries_a);
    println!("num_nz_entries_b: {}", num_nz_entries_b);
    println!("num_nz_entries_c: {}", num_nz_entries_c);
    ...
    (
        ...
        num_vars,
        num_inputs,
        max(num_nz_entries_a, max(num_nz_entries_b, num_nz_entries_c))
    )
}
...
fn lc_to_v(lc: &Lc, const_id: usize, trans: &HashMap<Var, usize>) -> Vec<Variable> {
    ...
    // println!("LC_LEN: {}", lc.monomials.len());

...
enum Backend {
    ...
    R1cs {
        ...
        #[arg(long, default_value = "spartan-setup")]
        action: ProofAction,
...
fn main() {
    ...
    let path_buf = PathBuf::from(format!("../zok_tests/benchmarks/{}.zok", benchmark_name));
    ...
    match options.backend {
        ...
        Backend::R1cs {
            ...
            compiler_time = compiler_start.elapsed();
            ...
            match action {
                ...
                ProofAction::SpartanSetup => {
                    ...
                    let to_field = |i: Integer| Value::Field(FieldV::new(i, Integer::from_str_radix("7237005577332262213973186563042994240857116359379907606001950938285454250989", 10).unwrap().into()));
                    let to_bv = |i: usize, w: usize| Value::BitVector(BitVector::new(Integer::from(i), w));
                    ...
                    while buffer != "END".to_string() {
                        ...
                        let var = split[0].to_string();
                        let val = to_bv(split[1].parse::<usize>().unwrap(), 32);
                        // let val = to_field(Integer::from_str_radix(&split[1], 10).unwrap());
                        /*let val = match split[1].as_str() {
                            "field" => to_field(val),
                            "u64" => to_bv(val, 64),
                            "u32" => to_bv(val, 32),
                            "u16" => to_bv(val, 16),
                            "u8" => to_bv(val, 8),
                            _ => { panic!("Unsupported input type: {}!", split[1]) }
                        };*/
                    ...
                    write_data::<_, _>(prover_key.clone(), verifier_key.clone(), &prover_data, &verifier_data)
                        .unwrap();
                    let (gens, inst, proof, comm) = prove::<_>(prover_key, &input_map).unwrap();
                    let _ = verify::<_>(verifier_key, &input_map, &gens, &comm, proof);
                }
